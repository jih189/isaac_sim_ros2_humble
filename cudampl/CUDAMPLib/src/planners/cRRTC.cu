...
namespace CUDAMPLib
{
        ...
        : BasePlanner(space)
    {
        ...
        cudaMalloc(&d_start_tree_size_, sizeof(int));
        ...
        cudaMalloc(&d_goal_tree_size_, sizeof(int));
    ...
    void cRRTC::solve(BaseTerminationPtr termination_condition)
    {
        ...
        cudaMemset(d_start_tree_size_, 0, sizeof(int));
        ...
        cudaMemset(d_goal_tree_size_, 0, sizeof(int));
        ...
        // set the start and goal tree size to 1
        int start_tree_size = 1;
        int goal_tree_size = 1;
        cudaMemcpy(d_start_tree_size_, &start_tree_size, sizeof(int), cudaMemcpyHostToDevice);
        cudaMemcpy(d_goal_tree_size_, &goal_tree_size, sizeof(int), cudaMemcpyHostToDevice);
...
extern "C" __global__ void cRRTCKernel(float * d_start_tree_configurations, float * d_goal_tree_configurations, int * d_start_tree_parent_indexs, int * d_goal_tree_parent_indexs, int * d_start_tree_size, int * d_goal_tree_size, float * d_sampled_configurations) {
    ...
    kernel_code += R"(
        ...
        if (tid == 0)
        {
            ...
            {
                kernel_code += "            printf(\"%f \", d_sampled_configurations[i * " + std::to_string(dim_) + " + " + std::to_string(j) + "]);\n";
            }
            kernel_code += "            printf(\"\\n\");\n";
    kernel_code += R"(
        }
